import {Coord, HasPos, PathfinderReturn, TravelData, TravelState, TravelToOptions} from './Traveler_Types';
...
export class Traveler {
        ...
        const ret = Game.map.findRoute(origin, destination, {
            routeCallback: (roomName: string) => {
                ...
                if (!options.allowSK && !Game.rooms[roomName]) {
                    if (parsed.length === 0) { parsed = /^[WE]([0-9]+)[NS]([0-9]+)$/.exec(roomName) as any; }
    ...
    private static deserializeState(travelData: Partial<TravelData>, destination: RoomPosition): TravelState {
    ...
    private static serializeState(creep: Creep, destination: RoomPosition, state: TravelState, travelData: Partial<TravelData>) {
...
Creep.prototype.travelTo = function(destination: RoomPosition|{pos: RoomPosition}, options?: TravelToOptions): number {
